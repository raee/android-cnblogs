...
//            String avatar = getAvatar(element.select(".pfs").attr("src")); // 头像地址
            ...
            String comment = Utils.getCount(Utils.getNumber(element.select(".searchItemInfo-comments").text())); // 评论
            String views = Utils.getCount(Utils.getNumber(element.select(".searchItemInfo-views").text())); // 阅读
            String likes = Utils.getCount(Utils.getNumber(element.select(".searchItemInfo-good").text())); // 点赞或者是推荐
            String date = Utils.getDate(element.select(".searchItemInfo-publishDate").text()); // 发布时间
